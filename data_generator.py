...
class DataGenerator():
    ...
    def generator(self, X = None, y = None, minmax = True,
        ...
        if la == -1:
            ...
            test_with_anomaly = np.append(test_ind, anomaly_ind)

            np.random.shuffle(test_with_anomaly)
            X_train = X[train_ind]
            X_test = X[test_with_anomaly]
            y_train = y[train_ind]
            y_test = y[test_with_anomaly]
            # minmax scaling
            if minmax:
                scaler = MinMaxScaler().fit(X_train)
                X_train = scaler.transform(X_train)
                X_test = scaler.transform(X_test)
            assert y_train.sum() == 0, "Anomaly in train!"
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = self.test_size, shuffle = True,
                                                            stratify = y)
