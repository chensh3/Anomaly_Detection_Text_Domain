...
def train_supervised(name,train, test):
    ...
    ho_train_data=h2o.H2OFrame(train_data)
    ho_train_data["label"]=ho_train_data["label"].asfactor()
    test_data=test.drop(columns=["short_description","category"])
    print("start setup")
    start = perf_counter
    aml=h2o.automl.H2OAutoML(max_models=10,balance_classes=True, seed=42)
    ...
    aml.train(training_frame=ho_train_data,y='label')
